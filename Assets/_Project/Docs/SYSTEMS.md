...
> **번호는 고정한다** — §16.1이 `§15.6-1`을, §16.3이 `§15.6-2`를 참조하므로 재번호 시 교차 참조가 깨진다.
> 해소된 항목은 지우지 않고 ✅ + 처리 위치를 남긴다 (D7 세션 14 현행화).

**남은 것 (제출 전 처리 대상)**
2. **토글 OFF 폴백 실주행** — `Awake`가 플레이 모드에서만 도므로 미검증. **안전판의 유일한 구멍.**
   D7 세션 1이 소프트락 원인(스폰 링 확대)은 고쳤으나(§16.2) **실주행 확인은 여전히 사람 대기** — 절차 §16.3-5
4. **보스 P1 실플레이 검증** — D3부터 이월 중
7. 적 3종(Beholder·Djinn·Wizard)이 파랑·시안 계열 — 예약 색역(전공색 파랑)과 겹친다.
   §10.4 개정이 아니라 **스프라이트 교체/틴트로 처리할 사안** (기획 판단 대기)
8. 영상 촬영 — D7 몫

**해소됨**
1. ✅(부분) **엘리트·보스 P2 마젠타화 경로** — 컬러 스프라이트 위에서 곱셈 틴트가 안 먹는 문제를
   **틴트 + 마젠타 오라 링 + 공격색 분산**으로 우회 (D7 세션 1, §16.1 마지막 항목).
   ⚠️ **눈으로 본 검증은 아직 없다** — 제출 문서 #3·#4가 마젠타 예측탄을 게임의 얼굴로 쓰므로 실물 대조 권장
3. ✅ **보스 P2 `PATTERN: YOU`** — D7 세션 1 구현 (§16.1). 실플레이 검증은 4번과 함께
5. ✅ 화면 밖 위협 보정(가장자리 화살표) — D7 세션 4 (§16.6)
6. ✅ Dungeon Tileset 팩 URL·라이선스 — Franuka Dungeon Asset Pack으로 특정, D7 세션 5 (`CREDITS.md` ⚠️ 해제)
9. ✅ 오디오 전무 → 충족 + 무음 결함 수정 — D7 세션 2·7 (§16.4)
10. ✅ UI 한국어 교체 — D7 세션 3 (§16.5)
11. ✅ 결과 화면 가운뎃점(`·`)이 □로 렌더링 — 한글 폰트에 U+00B7 없음. `|`로 교체, D7 세션 14 (§16.8)
...
- 배선: `Luddite/Setup/오디오 배선 (§12)` (멱등 — 누락 클립은 LogError로 드러남).
  **이 빌더가 AudioListener도 보장한다** (D7 세션 13) — 씬 리스너가 0개면 AudioSource가 정상이어도 전부 무음인데
...
- 실측 검증(D7 세션 13): 리스너 1개(Main Camera) / AudioSource 2개(`spatialBlend=0`) / 클립 배선 누락 0/10 /
...
- 이로써 **추적 카메라의 조건부 동반 계약("보정 없는 추적 금지")이 완결**

### 16.7 Dungeon Tileset 출처 확정 (D7 세션 5 — §15.6-6 해소)
- 팩 특정: **Franuka「Dungeon Asset Pack」v1.1** (https://franuka.itch.io/dungeon-asset-pack) — 페이지 구성(16×16 타일·상자 3종·잠긴 문·횃불)과 승격 파일·PPU 16 일치, 원본 드라이브가 Franuka 컬렉션. 라이선스는 타 Franuka 팩과 동일(상업 ⭕ / 재배포 ❌). CREDITS·SUBMIT_4 반영
- **§15.6 잔여는 이제 2(토글 OFF 실주행)·4(보스 P1 실플레이)·7(적 색역)뿐** — 전부 사람 판단·실플레이 영역

### 16.8 폰트 글리프 결손 전수 검사 (D7 세션 14 — §15.6-11 해소)
- **증상**: 결과 화면 `SummaryLine`의 구분자 가운뎃점(`·` U+00B7)이 **□(U+25A1)로 렌더링**.
  한글 폰트 `x10y12pxDenkiChipHangul SDF`(글리프 3,005자)에 해당 문자가 없고,
  **폴백 `LiberationSans SDF - Fallback`은 글리프가 1개뿐이라 구제되지 않는다**
- **수정**: `UI/ResultProfile.cs`의 구분자를 `·` → `|` (폰트 보유 확인 완료. AI 발화 터미널체 §10.5에도 부합)
- **전수 검사** (같은 결손이 더 없는지 — 폰트가 한글 전체가 아닌 3,005자 부분집합이라 필요했다):
  | 대상 | 범위 | 결손 |
  |---|---|---|
  | 씬에 구운 TMP 텍스트 | 33개 | **0종** |
  | SO 표시 문자열 (한글 원문) | 28애셋 전 string 필드 | **0종** |
  | 코드 하드코딩 리터럴 | 37파일 / 149개 | **0종** (`·` 수정 후) |
- 검사 방법은 `TMP_FontAsset.HasCharacter()` 순회 — 재발 시 같은 방식으로 즉시 재검 가능.
  **새 한글 문구를 넣을 때는 이 폰트에 글자가 있는지 확인할 것** (없으면 조용히 □가 된다)

### 16.9 맵 품질 + 플레이어 UI 4종 (D7 세션 15 — 사람 요구 목록)

**맵 타일링·소품** (`Editor/DungeonSetupTools.cs`)
- 바닥 베이스 = **타일 113**(실측 표준편차 0.000, 완전 균일). ⚠️ 이전 96은 y6~y11에 얼룩이 있어
  32×18 방에서 576번 반복되며 격자로 보였다 — **베이스는 반드시 무늬 없는 타일로 둔다**
- 얼룩은 95~100을 격자 스냅 + 해시로 **밀도 10%** 결정론 배치. 배열 중복으로 가중치를 줘
  진한 타일(95=잉크 33%, 98=38%)은 드물게 뽑는다 (뭉치면 그물 무늬 → 되레 "타일링" 인상)
- 세로 벽 = 파생 타일 `Sprites/Dungeon/Generated/Wall_Side.png`. 원본 32는 y1~y4가 주황 띠라
  세로로 반복하면 1유닛마다 띠가 나와 사다리처럼 보인다. 생성은 `EnsureDerivedTiles()`(멱등)
- 소품은 **벽 따라 고정 슬롯 8개** 대칭 배치 (난수 흩뿌리기 폐기, 시드 자체를 제거)
- 🔴 기둥은 **Column_3 하나로 통일** — Column_5·6은 한색(파랑=전공색 예약 침범),
  Column_2는 수치상 난색이나 화면에서 창백한 청백색으로 읽힌다(캡처로 확인)

**HUD** (`Editor/HudSetupTools.cs`) — 좌상단 HP 바 / 우상단 미니맵→그 아래 AI 미니 패널 / 우하단 무기·탄약
- 탄창: `PlayerStatsSO._magazineSize`(30) · `_reloadDuration`(1.2s). 탄약 총량 무한, 탄창만 소모
- 상태는 **`IWeapon`에** 있다(`MagazineSize`/`AmmoRemaining`/`IsReloading`/`ReloadProgress01`) —
  D8 전공별 무기로 갈아끼워도 `UI/AmmoCounter`는 무변경. UI는 읽기만 한다(규칙 7)
- 아이콘은 플레이어 투사체 `FireballBig` 재사용 — 신규 에셋 0
- `UI/Minimap` — 아이콘 위치는 빌더가 Room 실좌표에서 굽는다(체인을 바꾸고 재실행하면 따라옴).
  **답사 표시를 상태로 들지 않는다**: 체인이 선형·전진 전용이라 "지나온 방 = 인덱스가 작은 방"이
  항상 성립 → 재플레이 초기화 문제가 사라진다 (HudPanel은 Combat에서만 켜져 OnEnable 초기화가 성립 안 함)

**상자** (`Core/Chest.cs`, `UI/PanelPopIn.cs`)
- `DungeonConfig._autoOpenChest = false` / `_chestInteractRadius = 2.2`.
  반경 안에서 **E키**를 눌러야 열린다 → 인터벌 진입 시점을 플레이어가 쥔다
- "[E] 열기" 월드 스페이스 안내 + 상자 팝 + 인터벌 패널 `PanelPopIn`
- ⚠️ **두 연출 모두 unscaled 시간.** 인터벌은 timeScale 0이라 scaled면 첫 프레임에서 멈춘다
- ⚠️ `Chest._autoOpen`의 **씬 직렬화 기본값도 SO와 맞춰 굽는다** — 안 맞추면
  `DungeonManager.Configure`가 돌기 전(에디터 스모크 등)에 자동 오픈 true가 살아난다

**함께 고친 것**: 플레이어를 `RunStarted`에서만 시작방으로 옮기고 있어 **타이틀 배경이 폴백 아레나**였다
→ `DungeonManager.Awake`에서도 시작방에 세운다. 토글 OFF면 이 경로를 타지 않는다.
(아레나 바닥 `Floors1`은 단일 스프라이트 1장이라 변형 섞기가 불가능하고 다른 팩이라 던전 타일을 섞으면
일관성이 깨진다 → 폴백 경로 자체는 손대지 않았다)

- 검증: 던전 루프 스모크 **54/54**(수동 오픈 4건 신규). 실플레이 남은 것 = E키 실반응·HUD 실렌더·재장전 체감
