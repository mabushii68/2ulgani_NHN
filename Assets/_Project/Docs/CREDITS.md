...
| **Dungeon Tileset pack** — `Sprites/Dungeon/` 7파일 (`DungeonTileset` 448×96 / `Chest_Small` / `Chest_Large` / `Door_Front` / `Door_Side` / `Torch_Front` / `Torch_Side`) | ⚠️ **URL 사람 확인 필요** — `Sprites.v2/Environment/DungeonTileset/`에서 승격. 원본 드라이브(`G:\내 드라이브\RPG 픽셀 아트\`) 기준 Franuka 컬렉션으로 추정되나 팩 페이지를 특정하지 못했다 | ⚠️ **확인 필요** (Franuka 독자 라이선스로 추정) | 던전 방 바닥·벽 타일(아레나 교체), 문(방 락인), 상자(인터벌 대체), 횃불(방 테마 조명) | `(D5 커밋)` |
